 ...
 *//******/ (function() { // webpackBootstrap
 ...
 var nativeProps = ['autofocus', 'class', 'disabled', 'id', 'multiple', 'name', 'placeholder', 'required', 'data-select-group', 'data-ajax-handler', 'data-change-handler']; // SD CHANGE
 ...
 var VirtualSelect = /*#__PURE__*/function () {
   ...
   virtual_select_createClass(VirtualSelect, [{
     ...
     value: function setProps(params) {
       ...
       this.optionAutoHeight = options.optionAutoHeight; // SD CHANGE
       /** @type {string[]} */
     ...
     value: function setDefaultProps(options) {
       var defaultOptions = {
         ...
         emptyValue: '',
         searchDelay: 300,
         optionAutoHeight: false, // SD CHANGE
       };
     ...
     value: function setEleProps() {
       ...
       $ele.getDisabledOptions = VirtualSelect.getDisabledOptionsMethod; // SD CHANGE
     ...
     value: function setValueMethod(newValue, silentChange) {
       ...
       this.options.forEach(function (d) {
         ...
         if (valuesMapping[d.value] === true && !d.isGroupTitle) { // SD CHANGE set value even if disabled
           // eslint-disable-next-line no-param-reassign
           d.isSelected = true;
     ...
     value: function setValueText() {
       ...
       if (showAllText && this.hideValueTooltipOnSelectAll) {
         ...
         if (aggregatedValueText === '') {
           ...
           $valueText.innerHTML = aggregatedValueText.replace(/(^,)|(,$)/g, ''); // SD CHANGE
     ...
     value: function setOptionsHeight() {
       ...
       // SD CHANGE make change here
       if(this.optionAutoHeight){
         this.$optionsList.style.height = "auto";
       }
       else{
         this.$optionsList.style.height = "".concat(this.optionHeight * this.visibleOptionsCount, "px");
       }
     ...
     value: function setOptionsContainerHeight(reset) {
       ...
       // SD CHANGE
       if(this.optionAutoHeight){
         DomUtils.setStyle(this.$optionsContainer, 'max-height', 'none');
       }
       else{
         DomUtils.setStyle(this.$optionsContainer, 'max-height', optionsHeight); 
       }    
     ...
     value: function setOptionAttr() {
       ...
       var optionHeight; // SD CHANGE
       if(this.optionAutoHeight){
         optionHeight = 'auto';
       }
       else{
         optionHeight = "".concat(this.optionHeight, "px");
       }
       // end
   ...
   {
     key: "getDisabledOptions", // SD CHANGE
   ...
   },
   {
     ...
     value: function openDropbox(isSilent) {
       ...
       //this.setVisibleOptions(); // SD CHANGE -- BUG in chrome when i call it
     ...
     value: function setPropsFromSelect(props) {
       ...
       var getNativeOptions = function getNativeOptions($container) {
         ...
         $options.forEach(function ($option) {
           ...
           if ($option.getAttribute('selected') =='') {     // SD CHANGE  
   ...
   {
     key: "getDisabledOptionsMethod", // SD CHANGE
