...
Im Tab 'Daten' können Sie auch auswählen, ob Sie eine lokale JSON-Datei (also eine 'hochgeladene') oder eine Online-Datenbank verwenden wollen. Beide Werte sind im System konfiguriert und es wird empfohlen, die lokale Datei zu verwenden, damit die Website nicht externe URLs aufrufen muss.
